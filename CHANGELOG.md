...
- Make the app readable on phone, tablet, and laptop: rail from 768px, wrapping brand chips, one-line model titles, labeled Install/Pin/Search, and a Search tab that focuses the field. Header controls are all 44px; Install is ink, Search is blue, Pin stays cream (red when pinned). Preview uses a tape width the device actually has (Brady is 12.7 mm, not a fake 12 mm).
