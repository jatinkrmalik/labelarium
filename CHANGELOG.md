...
- Make the app readable on phone, tablet, and laptop: rail from 768px, wrapping brand chips, one-line model titles, labeled Install/Pin/Search, and a Search tab that focuses the field. Header controls are all 44px; Install is tape yellow on home and device pages, Search is blue, Pin stays cream (red when pinned). Preview uses a tape width the device actually has (Brady is 12.7 mm, not a fake 12 mm). Long preview text keeps letter height and scrolls the strip (like the printer) instead of shrinking 12 mm tape to a sliver. Frame artwork is 9-sliced around the typed text so a long label stretches the middle and leaves the end caps un-squashed; DYMO sample ABC stays out of the letters. On home, Install sits to the right of the brand chips instead of doubling with a top bar. Cancelling the browser install prompt no longer hides the button.
